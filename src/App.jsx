...
const ONLINE_CLIENT_ID_KEY = "impostor-online-client-id";
const ONLINE_NAME_KEY = "impostor-online-player-name";
const ONLINE_FIREBASE_URL_KEY = "impostor-online-firebase-url";
const ONLINE_POLL_MS = 1200;
...
function getOnlineClientId() {
  ...
  window.localStorage.setItem(ONLINE_CLIENT_ID_KEY, next);
  return next;
}

function createOnlineRoomId() {
...
function normalizeOnlineRoom(rawRoom, fallbackRoomId, clientId, playerName) {
  const now = Date.now();
  if (rawRoom && typeof rawRoom === "object") return rawRoom;
  return {
    id: fallbackRoomId,
    hostId: clientId,
    createdAt: now,
...
function getOnlinePlayerNames(room) {
  return getOnlinePlayers(room).map((player) => player.name).filter(Boolean);
}

function countVotes(votes = {}) {
  return Object.values(votes || {}).reduce((acc, value) => {
    if (!value) return acc;
...
function getVoteWinner(votes = {}, fallback = "continue") {
  const counts = countVotes(votes);
  const entries = Object.entries(counts).sort((a, b) => b[1] - a[1]);
  if (entries.length === 0) return fallback;
  if (entries.length > 1 && entries[0][1] === entries[1][1]) return fallback;
  return entries[0][0];
}
...
function normalizeOnlineIdentity(value) {
  return String(value ?? "")
    .normalize("NFKC")
    .replace(/[​-‏‪-‮⁠﻿­͏︀-️]/g, "")
    .replace(/ /g, " ")
...
function OnlineMode({ onExit, isMobile }) {
  ...
  const storedOnlineName = typeof window === "undefined" ? "" : window.localStorage.getItem(ONLINE_NAME_KEY) || "";
  const selfNameCandidates = [me?.name, name, storedOnlineName, room?.players?.[clientId]?.name]
    .map(normalizeOnlineIdentity)
    .filter(Boolean);
  const isSelfName = (value) => selfNameCandidates.includes(normalizeOnlineIdentity(value));
  const hostPlayer = players.find((player) => player.id === room?.hostId) || null;
  const isHost = room?.hostId === clientId || Boolean(hostPlayer?.name && isSelfName(hostPlayer.name));
  const normalizedMeName = selfNameCandidates[0] || "";
  const myAssignment = room?.round?.assignments?.find((assignment) => isSelfName(assignment.player)) || null;
  const currentTurnPlayer = room?.round?.assignments?.[room?.turnIndex || 0]?.player || "";
  const normalizedTurnPlayer = normalizeOnlineIdentity(currentTurnPlayer);
  const normalizedMyAssignmentPlayer = normalizeOnlineIdentity(myAssignment?.player || "");
  const isMyTurn = room?.phase === "clue" && Boolean(
    isSelfName(currentTurnPlayer) ||
    (normalizedMyAssignmentPlayer && normalizedTurnPlayer === normalizedMyAssignmentPlayer)
  );
  ...
  function showOnlineToast(message) {
    setOnlineToast(message);
    if (toastTimerRef.current) window.clearTimeout(toastTimerRef.current);
    toastTimerRef.current = window.setTimeout(() => setOnlineToast(""), 2600);
  }
  ...
  function shouldCommitOnlineAction(actionKey) {
    const timestamp = Date.now();
    const last = onlineActionCommitRef.current || { key: "", at: 0 };
    if (last.key === actionKey && timestamp - last.at < 900) return false;
    onlineActionCommitRef.current = { key: actionKey, at: timestamp };
    return true;
  }

  function handleDecisionPressStart(value) {
    markDecisionChoice(value);
  }

  function handleDecisionCommit(value) {
    markDecisionChoice(value);
    if (!shouldCommitOnlineAction(`decision:${value}`)) return;
    voteDecision(value);
  }

  function handleSuspectPressStart(player) {
    markSuspectChoice(player);
  }

  function handleSuspectCommit(player) {
    markSuspectChoice(player);
    if (!shouldCommitOnlineAction(`suspect:${player}`)) return;
    voteSuspect(player);
  }
  ...
  useEffect(() => {
    if (room?.phase !== "decision") {
      ...
      setVoteFeedback((prev) => (prev.decision ? { ...prev, decision: "" } : prev));
    }
  ...
  useEffect(() => {
    if (room?.phase !== "suspect_vote") {
      setLocalSuspectVote("");
      setVoteFeedback((prev) => (prev.suspect ? { ...prev, suspect: "" } : prev));
    }
  ...
  async function submitClue() {
    ...
    if (!clue) return;
    const nextClues = [...(room.clues || []), { player: me.name, clue, at: Date.now(), roundNumber: room.roundNumber || 1 }];
    const isLastTurn = (room.turnIndex || 0) >= (room.round.assignments.length - 1);
    await patchRoom({
      clues: nextClues,
      turnIndex: isLastTurn ? room.turnIndex : (room.turnIndex || 0) + 1,
      phase: isLastTurn ? "decision" : "clue",
      phaseEndsAt: Date.now() + (isLastTurn ? ONLINE_DECISION_SECONDS : ONLINE_TURN_SECONDS) * 1000,
      decisionVotes: isLastTurn ? {} : room.decisionVotes || {},
    });
    setClueText("");
  }
  ...
  async function sendChat() {
    ...
    if (!text || !me) return;
    const nextChat = [...(room.chat || []), { player: me.name, text, at: Date.now() }].slice(-80);
    await patchRoom({ chat: nextChat });
    setChatText("");
  }
  ...
  async function voteDecision(value) {
    ...
    try {
      ...
      showOnlineToast("Could not save vote");
      throw err;
    }
  }
  ...
  async function submitImpostorGuess() {
    ...
    const correct = guess.localeCompare(room.round.character.name, undefined, { sensitivity: "accent" }) === 0;
    await patchRoom({
      impostorGuess: guess,
      phase: correct ? "game_over" : "host_override",
      phaseEndsAt: correct ? null : Date.now() + ONLINE_HOST_OVERRIDE_SECONDS * 1000,
      winner: correct ? { side: "impostor", reason: "The impostor guessed the word." } : null,
    });
  }

  async function hostOverrideWin() {
    if (!isHost) return;
    await patchRoom({ phase: "game_over", phaseEndsAt: null, winner: { side: "impostor", reason: "Host accepted the impostor guess." } });
  }

  async function hostNoOverride() {
    if (!isHost) return;
    await patchRoom({ phase: "game_over", phaseEndsAt: null, winner: { side: "crewmates", reason: "The impostor did not guess the word." } });
  }
  ...
  const selectedVoteButtonStyle = {
    background: "#16a34a",
    color: "white",
    outline: "4px solid #86efac",
    boxShadow: "0 0 0 6px rgba(34,197,94,0.22), 0 16px 30px rgba(21,128,61,0.25)",
  ...
  return (
    <div style={{ minHeight: "100vh", padding: isMobile ? 14 : 28, ...NO_SELECTION_STYLE, background: "radial-gradient(circle at top left, rgba(59, 130, 246, 0.35), transparent 28%), linear-gradient(135deg, #eff6ff 0%, #f5f3ff 100%)", color: "#111827" }}>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", display: "grid", gap: 16, paddingTop: 0 }}>
        ...
        {joined && room && (
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(0, 1.55fr) minmax(280px, 0.8fr)", gap: 16 }}>
            <div style={{ display: "grid", gap: 16 }}>
              ...
              {room.phase !== "lobby" && (myAssignment || room.phase === "game_over") && (
                <div style={{ ...panelStyle, display: "grid", gap: 14 }}>
                  ...
                  <button onPointerDown={() => setCardRevealed(true)} onPointerUp={() => setCardRevealed(false)} onPointerLeave={() => setCardRevealed(false)} onClick={() => setCardRevealed((prev) => !prev)} style={{ minHeight: 190, border: 0, borderRadius: 18, display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center", lineHeight: 1.15, background: cardRevealed ? (myAssignment.isImpostor ? "linear-gradient(145deg, #020617 0%, #7f1d1d 55%, #dc2626 100%)" : "linear-gradient(145deg, #dbeafe 0%, #bfdbfe 100%)") : "linear-gradient(145deg, #312e81 0%, #4f46e5 55%, #111827 100%)", color: cardRevealed ? (myAssignment.isImpostor ? "#fee2e2" : "#172554") : "white", fontWeight: 950, fontSize: 26, cursor: "pointer", ...NO_SELECTION_STYLE }}>
                    {cardRevealed ? (
                      <div>
                        ...
                        <div>{myAssignment.word}</div>
                        {myAssignment.isImpostor && myAssignment.hint && <div style={{ fontSize: 16, marginTop: 8 }}>Hint: {myAssignment.hint}</div>}
                      </div>
                    ) : "Hold / tap to reveal"}
                  </button>
                  )}
                  ...
                  {(room.clues || []).length > 0 && (
                    <div style={{ display: "grid", gap: 8 }}>
                      <div style={{ fontWeight: 950, color: "#172554" }}>Clues</div>
                      {(room.clues || []).map((clue, index) => <div key={`${clue.at}-${index}`} style={{ padding: 10, borderRadius: 12, background: "#f8fafc", fontWeight: 800 }}>{clue.player}: {clue.clue}</div>)}
                    </div>
                  )}
                  ...
                  {room.phase === "host_override" && (
                    <div style={{ display: "grid", gap: 10 }}>
                      <div style={{ fontWeight: 950 }}>Impostor guessed: {room.impostorGuess || "No guess"}</div>
                      {isHost ? <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><button onClick={hostOverrideWin} style={{ ...buttonStyle, background: "#16a34a" }}>Give impostor win</button><button onClick={hostNoOverride} style={{ ...buttonStyle, background: "#111827" }}>Crewmates win</button></div> : <div style={{ fontWeight: 800, color: "#475569" }}>Host can approve a typo for 10 seconds.</div>}
                    </div>
                  )}
            ...
            <div style={{ ...panelStyle, display: "grid", gap: 10, alignSelf: "start" }}>
              <div style={{ fontWeight: 950, color: "#172554" }}>Chat</div>
...
export default function App() {
  ...
  if (onlineMode) {
    return <OnlineMode isMobile={isMobile} onExit={() => setOnlineMode(false)} />;
  }
